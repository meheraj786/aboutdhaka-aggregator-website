...
const DHAKA_AREAS = [
  "Adabor", "Badda", "Bangsal", "Bimanbandar", "Cantonment", "Chawkbazar",
...
export async function getAreas() {
  try {
    await dbConnect();
    const areas = await Area.find({}).select("_id name").sort({ name: 1 }).lean();
    return areas as { _id: string; name: string }[];
  } catch (error) {
...
export async function seedAreas() {
  try {
    ...
    const existingAreas = await Area.find({ name: { $in: DHAKA_AREAS } }).select("name");
    const existingNames = new Set(existingAreas.map(a => a.name));
    
    const areasToSeed = DHAKA_AREAS
      .filter(name => !existingNames.has(name))
      .map(name => ({ name }));

    if (areasToSeed.length > 0) {
      await Area.insertMany(areasToSeed);
      return { success: true, message: `Seeded ${areasToSeed.length} new areas.` };
    }
