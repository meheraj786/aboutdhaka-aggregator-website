	...
	const filterSections = useMemo(() => [
		{
			...
			multiSelect: true,
			searchable: true,
			options: (areasData || []).map((a: { name: string }) => ({ label: a.name, value: a.name })),
			selected: params.areas || [],
			onChange: (v: string[]) => setParams(prev => ({ ...prev, areas: v, page: 1 })),
		},
		{
			title: "Category",
			multiSelect: true,
			...
			onChange: (v: string[]) => setParams(prev => ({ ...prev, categories: v, page: 1 })),
		},
				...
				<div className="max-w-7xl mx-auto">
					...
					<div className="flex flex-col lg:flex-row gap-10">
						<FilterSidebar 
							...
							onClearAll={() => setParams(prev => ({ ...prev, areas: [], categories: [], search: "", page: 1 }))} 
						/>
							...
							{isLoading ? (
								<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
									{[...Array(PAGE_SIZE)].map((_, i) => (
										<div key={i} className="h-72 animate-pulse rounded-[2rem] bg-slate-200" />
									))}
								</div>
							) : (
								<>
