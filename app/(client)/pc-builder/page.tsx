		...
		<div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] font-sans selection:bg-blue-100">
			<header className="max-w-7xl mx-auto px-6 pt-8 pb-4">
				...
				<p className="text-lg text-slate-600">
					Enter your workflow and get a precise hardware plan.
				</p>
			</header>
			...
			<main className="max-w-7xl mx-auto px-6 pb-20 grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 items-start">
				...
				<aside className="space-y-6 lg:sticky lg:top-8">
					<div
						className={cn(
							"bg-white rounded-[2.5rem] p-8 border relative overflow-hidden transition-all duration-500",
							aiResult
								? "border-blue-200 shadow-[0_20px_50px_rgba(59,130,246,0.15)]"
								: "border-slate-100 shadow-xl shadow-slate-200/50",
						)}
					>
						<div
							className={cn(
								"absolute -top-24 -right-24 w-48 h-48 rounded-full blur-3xl transition-colors",
								aiResult ? "bg-blue-100" : "bg-slate-50",
							)}
						/>
						<div className="relative z-10">
									...
									<div className="grid grid-cols-2 gap-6">
										<div className="flex items-start gap-3">
											...
											<div>
												...
												<p className="text-xs font-black text-slate-900">
													{displaySpecs.cores}C/{displaySpecs.threads}T
												</p>
											</div>
										</div>
										<div className="flex items-start gap-3">
										...
										<div className="flex items-start gap-3">
											...
											<div>
												...
												<p className="text-xs font-black text-slate-900">
													{displaySpecs.ssd >= 1024
														? `${displaySpecs.ssd / 1024}TB`
														: `${displaySpecs.ssd}GB`}
												</p>
											</div>
										</div>
								...
								{!aiResult ? (
									<div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
										...
										<button
											...
											className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-blue-700 transition-all disabled:bg-slate-300"
										>
											{isAnalyzing ? (
									...
									<button
										...
										className="text-[10px] font-bold text-slate-400 hover:text-red-500 uppercase flex items-center gap-1 mx-auto transition-colors"
									>
							...
							<div className="mt-6 flex gap-4 p-5 rounded-2xl bg-blue-50/50 border border-blue-100/50">
								...
								<p className="text-[11px] font-medium text-blue-900/70 leading-relaxed italic">
									"{displaySpecs.reason}"
								</p>
							</div>
						</div>
					...
					<div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
						...
						<div className="space-y-2 mb-6">
							<div className="flex justify-between text-xs">
								<span className="text-slate-500">Usage Case</span>
							...
							<div className="flex justify-between text-xs">
								<span className="text-slate-500">Storage Profile</span>
								<span className="font-bold text-slate-900">
									{state.storageNeeds}
								</span>
							</div>
						...
						<button
							type="button"
							className="w-full py-3 bg-[#0F172A] text-white font-bold rounded-xl hover:bg-blue-600 transition-all shadow-lg shadow-slate-200 flex items-center justify-center gap-2"
						>
