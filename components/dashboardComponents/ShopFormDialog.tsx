...
import { useCreateComponent, useUpdateComponent } from "@/hooks/usePCComponents";
...
function buildDefaults(editing: IPCComponentPopulated | null): PCComponentInput {
  if (!editing) {
    return {
      name: "",
      brand: "",
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Budget Tier" error={errors.minBudgetTier?.message}>
                <Select
                  ...
                  onValueChange={(v) =>
                    setValue("minBudgetTier", v as PCComponentInput["minBudgetTier"])
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                ...
                <Field label="Socket Type" error={errors.socket?.message}>
                  <Select
                    ...
                    onValueChange={(v) =>
                      setValue(
                        "socket",
                        v === "" ? undefined : (v as PCComponentInput["socket"]),
                      )
                    }
                  >
                    <SelectTrigger>
