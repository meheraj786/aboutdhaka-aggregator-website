...
import { Loader2, Plus, X, Image as ImageIcon, Utensils, Trash2 } from "lucide-react";
...
import { type CreateRestaurantInput, createRestaurantSchema } from "@/validators/restaurants";
...
export function RestaurantFormDialog() {
	...
	const form = useForm<CreateRestaurantInput>({
		resolver: zodResolver(createRestaurantSchema) as Resolver<CreateRestaurantInput>,
			...
			<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-2">
								...
								<div key={field.id} className="flex gap-2 items-start">
									<div className="flex-1">
										<Input 
											...
											className={form.formState.errors.menu?.[index]?.name ? "border-destructive" : ""}
										/>
									</div>
									<div className="w-24">
										<Input 
											...
											placeholder="Price" 
											className={form.formState.errors.menu?.[index]?.price ? "border-destructive" : ""}
										/>
									</div>
									<Button 
										type="button" 
					...
					<section className="space-y-3">
						<FieldLabel className="text-xs uppercase tracking-wider text-muted-foreground">Amenities</FieldLabel>
							...
							{amenities.map((item) => (
								<Badge key={item} variant="secondary" className="pr-1 py-1">
									...
									<X 
										...
										onClick={() => form.setValue("amenities", amenities.filter(a => a !== item))} 
									/>
								</Badge>
							))}
						</div>
					...
					{/* Gallery Section */}
					<section className="space-y-3">
						<FieldLabel className="text-xs uppercase tracking-wider text-muted-foreground">Gallery Images</FieldLabel>
						<div className="flex gap-2">
							<Input 
								value={galleryInput} 
							...
							<div className="grid grid-cols-1 gap-2">
								{gallery.map((url, index) => (
									<div key={index} className="flex items-center gap-2 p-2 border rounded-md bg-muted/50">
										...
										<button 
											type="button" 
											onClick={() => form.setValue("gallery", gallery.filter(g => g !== url))}
											className="text-muted-foreground hover:text-destructive"
										>
											<X className="h-4 w-4" />
										</button>
